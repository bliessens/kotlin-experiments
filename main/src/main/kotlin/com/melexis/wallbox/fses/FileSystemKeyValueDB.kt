...
object DefaultFileNameGenerator : (Serializer, Serialized) -> String {
    override fun invoke(
        p1: Serializer,
        p2: Serialized,
    ): String = String.format("event-%03d.%s", p2.seqNr, p1.fileExtension(p2))
}
...
class FileSystemKeyValueDB(
    ...
    override fun persist(
        id: WallBoxId,
        events: List<AbstractWallBoxEvent>,
    ) {
        val eventDir = File(rootDir, id.toString()).also { if (!it.exists()) it.mkdir() }
        events
            .map { event -> serializer.serialize(event) }
...
fun File.sequenceNumber(): Int = this.name.substring(6, 9).toInt()
...
data class Serialized constructor(
    ...
    override fun compareTo(other: Serialized): Int = seqNr.compareTo(other.seqNr)
}
