...
class BinTree<T : Comparable<T>>(
    ...
    fun add(value: T): Boolean = add(root, value)
    ...
    private fun addLeft(
        ...
        if (node.left is None<T>) {
            node.left = Node(value = value)
            true
        } else {
            add(node.left as Node<T>, value)
        }
    ...
    private fun addRight(
        ...
        if (node.right is None<T>) {
            node.right = Node(value = value)
            true
        } else {
            addRight(node.right as Node<T>, value)
        }
