...
object DIContext {
    ...
    inline fun <reified T> get(): T = singletons[TypeInfo.typeOf(T::class)] as T
}

sealed interface TypeInfo {
    companion object {
        fun typeOf(type: KClass<*>): TypeInfo = UnNamedTypeInfo(type)
...
data class NamedTypeInfo(
    val type: KClass<*>,
    val name: String = "",
) : TypeInfo

data class UnNamedTypeInfo(
    val type: KClass<*>,
) : TypeInfo
...
inline fun <reified T> injectObject() =
    object : ReadOnlyProperty<Any?, T> {
        override fun getValue(
            thisRef: Any?,
            property: KProperty<*>,
        ): T = DIContext.singletons[TypeInfo.typeOf(T::class)] as T
    }
...
inline fun <reified T> injectObject(name: String = "") =
    object : ReadOnlyProperty<Any?, T> {
        override fun getValue(
            thisRef: Any?,
            property: KProperty<*>,
        ): T = DIContext.singletons[TypeInfo.typeOf(T::class, name)] as T
    }
...
class DIContextBuilder {
    ...
    inline fun <reified T> singletonOf(
        name: String = "",
        supplier: () -> T,
    ) {
